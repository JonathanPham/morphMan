...
def landmarking_bogunovic(centerline, base_path, approximation_method, algorithm, resampling_step, smooth_line, nknots,
    ...
    try:
        ...
        os.remove(base_path + "_landmark_bogunovic_%s_r_%.2f_f_%.1f_i_%i.particles" % (
            approximation_method, resampling_step, smoothing_factor, iterations))

        #os.remove(base_path + "_landmark_bogunovic_%s.particles" % approximation_method)
        #os.remove(base_path + "_landmark_bogunovic_%s_r_%.3f.particles" % (approximation_method, resampling_step))
    except:
        pass
    ...
    if landmarks is not None:
        ...
        #create_particles(base_path, algorithm, approximation_method, r=resampling_step)
        create_particles(base_path, algorithm, approximation_method, resampling_step, smoothing_factor,
                         iterations)
